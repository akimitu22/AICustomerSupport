...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    // QandA情報をコンテキストに変換
    const qaContext = kindergartenQA.map(q => `Q: ${q.question}\nA: ${q.answer}`).join('\n');
...
※発話者の会話に出てくる単語は基本的に幼稚園用語と考えて対応してください。たとえば、「えんちょう」は、「園長先生」か「延長保育(預かり保育)」の意味で。「がんしょ(う)」は、「眼症」や「顔症」などの医療用語ではなく、「入園願書」のことという具合に理解してください。,
            },
