...
function correctKindergartenTerms(text) {
  ...
  out = out.replace(/(?:^|[^一-龯ぁ-んァ-ン])えんちょう(?=$|[^一-龯ぁ-んァ-ン])/g, function(match) {
    return match.replace(/えんちょう/g, '園長');
  });
  ...
  for (const [wrong, right] of sortedEntries) {
    ...
    out = out.replace(pat, function(match) {
      // 先頭を保持して置換（短語用）
      if (wrong.length <= 2) {
        return match.replace(wrong, right);
      }
      return right;
    });
  }
  ...
  out = out
    ...
    .replace(/(\S+)(幼稚園|保育園)/g, function(match, p1, p2) {  // 幼稚園名の修正
