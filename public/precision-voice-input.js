  ...
  async start() {
    ...
    try {
      ...
      this.timers.activity = setInterval(() => {
        if (this.isRecording && !this.isSpeaking && Date.now() - this.startTime > 5000) {
          console.info('5秒間発話がありません');
          this._updateUI('音声が検出されていません...話しかけてください', false);
        }
      }, 5000);
      ...
      this.timers.uiUpdate = setInterval(() => {
        if (this.isRecording) {
          seconds++;
          if (this.isSpeaking) {
            this._updateUI(`録音中... (${seconds}秒)`, false);
          }
        }
      }, 1000);
...
// 録音がまだ始まっていなければ開始
 ...
 async _processRecording() {
   if (this.chunks.length === 0) {
     this._cleanupResources();
     this._updateUI('録音データがありません。もう一度お試しください。', true);
     return;
   }
