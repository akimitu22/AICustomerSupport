...
class PrecisionVoiceInput {
    ...
    try {
      ...
      this._boundVisibilityChange = async () => {
        if (document.visibilityState === 'visible' && this.audioContext?.state === 'suspended') {
          ...
          if (!resumed) {
            this._handleError(
              new Error('AudioContextを再開できませんでした。ページをリロードしてください。')
            );
          }
        }
      };
      ...
      const types = ['audio/webm;codecs=opus', 'audio/mp4', 'audio/ogg;codecs=opus', 'audio/mpeg'];
  ...
  async _processRecording() {
    ...
    try {
      ...
      const format =
        this.recorder && this.recorder.mimeType ? this.recorder.mimeType : this.mimeTypeFallback;
      ...
      console.info(
        `音声データサイズ: ${Math.round(base64Data.length / 1024)}KB, 録音時間: ${duration}秒`
      );
  ...
  _updateUI(message, isError) {
    ...
    if (this.button) {
      ...
      this.button.setAttribute(
        'aria-label',
        this.isRecording ? '音声入力を停止' : '音声入力を開始'
      );
  ...
  _handleError(error) {
    ...
    if (error.name === 'NotAllowedError') {
      ...
      error.message &&
      (error.message.includes('サーバーエラー') || error.message.includes('STT'))
    ) {
